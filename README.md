...
На C2-уровне система `Oregon` рассматривается как набор контейнеров: frontend-приложение, edge proxy, backend-сервисы, брокер сообщений, базы данных и observability-инструменты. Диаграмма ниже показывает, какие контейнеры входят в систему и по каким протоколам они взаимодействуют.
